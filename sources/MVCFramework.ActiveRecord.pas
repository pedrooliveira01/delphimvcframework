...
type
  ...
  public
    ...
    // TMVCActiveRecordHelper removido daqui
...
begin
  Result := SQLGenerator.CreateSelectSQL(fTableName, fMapNonTransientFields{fMap}, fPrimaryKeyFieldName, fPrimaryKeyOptions);
end;
...
class function TMVCActiveRecord.GetByPK<T>(const aValue: int64;
...
begin
  ...
  try
    ...
    Result := lList.First;//Result := lList.ExtractAt(0);
  finally
    lList.Free;
  end;
end;
...
begin
  ...
  else if aValue.IsType(TypeInfo(NullableDate)) then
  begin
    if aField.IsNull then
    begin
      aRTTIField.GetValue(Self).AsType<NullableDate>().Clear;
    end
    else
    begin
      aRTTIField.SetValue(Self, TValue.From<NullableDate>(aField.AsDateTime));
    end;
    Result := True;
  end
  else if aValue.IsType(TypeInfo(NullableDateTime)) then
  begin
    if aField.IsNull then
    begin
      aRTTIField.GetValue(Self).AsType<NullableDateTime>().Clear;
    end
    else
    begin
      aRTTIField.SetValue(Self, TValue.From<NullableDateTime>(aField.AsDateTime));
    end;
    Result := True;
  end
  else if aValue.IsType(TypeInfo(NullableTime)) then
  begin
    if aField.IsNull then
    begin
      aRTTIField.GetValue(Self).AsType<NullableTime>().Clear;
    end
    else
    begin
      aRTTIField.SetValue(Self, TValue.From<NullableTime>(aField.AsDateTime));
    end;
    Result := True;
  end
...
begin
  ...
  begin
    if not aValue.AsType<NullableDate>().HasValue then
    begin
    ...
    begin
      aValue := TValue.From<TDate>(aValue.AsType<NullableDate>().Value);
      Result := True;
    end;
  end
  else if aValue.IsType(TypeInfo(NullableTime)) then
  begin
    if not aValue.AsType<NullableTime>().HasValue then
    begin
    ...
    begin
      aValue := TValue.From<TTime>(aValue.AsType<NullableTime>().Value);
      Result := True;
    end;
  end
  else if aValue.IsType(TypeInfo(NullableDateTime)) then
  begin
    if not aValue.AsType<NullableDateTime>().HasValue then
    begin
    ...
    begin
      aValue := TValue.From<TDateTime>(aValue.AsType<NullableDateTime>().Value);
      Result := True;
    end;
  end
  else if aValue.IsType(TypeInfo(NullableUInt32)) then
